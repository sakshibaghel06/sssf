      ...
      <section className="container-seva py-16 md:py-20">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <Link href="/programs" className="rounded-full bg-maroon px-6 py-3 text-sm font-semibold text-ivory transition-transform hover:scale-[1.02] dark:bg-marigold dark:text-charcoal">
          ...
          {caseStudiesContent.length === 0 ? (
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-[2rem] border border-maroon/10 bg-white/60 p-8 dark:border-marigold/15 dark:bg-charcoal-soft/60">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-display text-2xl text-maroon dark:text-ivory">What each case study will include</h3>
                </div>
            ...
            <div className="grid gap-4">
              {caseStudiesContent.map((study) => (
                <article key={study.id} className="rounded-2xl border border-maroon/10 dark:border-marigold/15 p-6">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <span className="font-display text-xl text-maroon dark:text-ivory">{study.title}</span>
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.12em] text-sandalwood dark:text-ivory-soft/60">
                        {study.program ? <span>{study.program}</span> : null}
                        {study.year ? <><span className="mx-1">●</span><span>{study.year}</span></> : null}
                      </div>
                    </div>
                    <span className="rounded-full border border-maroon/10 px-4 py-2 text-[11px] font-semibold text-maroon dark:text-marigold">
                      {study.published ? "Published" : "Publication pending"}
                    </span>
                  </div>
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    <div>
                      <div className="flex items-center gap-2 text-sm text-sandalwood dark:text-ivory-soft/70">
                        <MapPin className="h-4 w-4 text-marigold" />
                        <span>{study.location || "Location pending"}</span>
                      </div>
                      <div className="mt-3 flex items-center gap-2 text-sm text-sandalwood dark:text-ivory-soft/70">
                      ...
                      <p className="mt-4 text-sm leading-relaxed text-sandalwood dark:text-ivory-soft/70">
                        <strong className="text-maroon dark:text-ivory">Problem:</strong> {study.problem}
                      </p>
                    </div>
                    <div className="rounded-2xl border border-maroon/10 dark:border-marigold/15 p-4">
                      <p className="text-sm leading-relaxed text-sandalwood dark:text-ivory-soft/70">
                        <strong className="text-maroon dark:text-ivory">Intervention:</strong> {study.intervention}
                      </p>
                    </div>
                  </div>
                </article>
