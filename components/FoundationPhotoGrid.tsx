    ...
    <section className="section-band py-16 md:py-20">
      <div className="container-seva">
        <Reveal>
          ...
          <h2 className="mt-5 font-display text-3xl text-maroon dark:text-ivory">{title}</h2>
        </Reveal>
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-3">
          {photos.map((photo, index) => (
            <Reveal key={photo.src} delay={Math.min(index * 45, 180)}>
              <figure className="group overflow-hidden rounded-xl bg-white/60 shadow-sm dark:bg-charcoal/30">
                ...
                <figcaption className="p-3 text-xs leading-relaxed text-sandalwood dark:text-ivory-soft/70">{photo.caption}</figcaption>
              </figure>
            </Reveal>
          ))}
        </div>
