  ...
  return (
    <div>
        ...
        {filters.map((filter) => (
          <button
            ...
            className={`rounded-full border px-4 py-2 text-xs font-semibold transition-colors ${
              activeFilter === filter
                ? "border-maroon bg-maroon text-ivory dark:border-marigold dark:bg-marigold dark:text-charcoal"
                : "border-maroon/15 text-maroon hover:border-maroon dark:border-marigold/20 dark:text-marigold"
            }`}
          >
            {filter}
          </button>
      ...
      {visiblePhotos.length === 0 && (
        <p className="mt-8 rounded-xl border border-dashed border-maroon/15 p-6 text-sm text-sandalwood dark:border-marigold/20 dark:text-ivory-soft/70">
