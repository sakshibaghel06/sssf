...
export default function ProgramCard({ icon: Icon, title, description, accent = "sky", slug }: ProgramCardProps) {
  ...
  return (
    <Link href={href} className="group flex min-h-[230px] flex-col rounded-2xl border border-maroon/10 bg-white/60 p-7 transition-all duration-300 hover:-translate-y-1 hover:border-marigold/50 hover:shadow-[0_14px_34px_rgba(11,15,140,0.1)] dark:border-marigold/15 dark:bg-charcoal-soft/60 dark:hover:shadow-[0_14px_34px_rgba(0,0,0,0.3)]">
      ...
      <p className="mt-2.5 flex-1 text-sm leading-relaxed text-sandalwood dark:text-ivory-soft/70">{description}</p>
      <span className="mt-5 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-maroon dark:text-marigold">Explore <span className="transition-transform duration-200 group-hover:translate-x-1">→</span></span>
    </Link>
  );
}
